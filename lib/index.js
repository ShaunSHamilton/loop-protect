...
const generateDateComparison = ({ t, id, line, ch, timeout, extra, isLoop } = {}) =>
  t.ifStatement(
...
const generateInside = ({ t, id, counterId, line, ch, timeout, extra, iterations, isLoop } = {}) =>
  iterations ?
  t.ifStatement(
    t.logicalExpression('&&',
      t.binaryExpression('===',
...
const protect = (t, timeout, extra, iterations) => path => {
  ...
  const isLoop = t.isWhileStatement(path) || t.isForStatement(path) || t.isDoWhileStatement(path);
  ...
  else if (body?.container?.type === 'VariableDeclaration' && body?.container?.declarations?.[0]?.init?.type === 'ArrowFunctionExpression') {
    path.insertBefore(counterVar);
    path.insertBefore(startVar);
  ...
  // For Class Declarations
    ...
    path.insertBefore(counterVar);
    path.insertBefore(startVar);
    body?.node?.body.find(node => node?.key?.name === 'render')?.body?.body.unshift(inside);
  }
};
